...
// const Serviceimages = ({ setActiveComponent }) => {
...
//     const CREATE_SERVICE_API_KEY = 'https://campusbuy-backend-nkmx.onrender.com/createservice';
//     const UPLOAD_IMAGES_API_KEY = 'https://campusbuy-backend-nkmx.onrender.com/uploadproductimage';
//     const { id } = useParams();
//     const serviceStore = JSON.parse(localStorage.getItem('editproduct'));
...
//         try {
//             openLoading();
//             const response = await axiosInstance.post(CREATE_SERVICE_API_KEY, serviceStore);
...
//             const uploadImages = await axiosInstance.put(`${UPLOAD_IMAGES_API_KEY}/${response.data._id}`, formData);
...
//                 <strong>Add Your service images. Note: Only *jpg, jpeg, png and svg* images allowed</strong>
//             </div>
...
//                             <ErrorMessage name="images" component="div" className="text-red-500" />
//                         </div>
//                         <button type="submit" className='py-2 px-4 border bg-[#FEBD69] rounded-lg ml-[100%] text-black'><strong>finish</strong></button>
//                     </Form>
//                 )}
//             </Formik>
//         </div>
...
const Serviceimages = ({ setActiveComponent }) => {
    ...
    const updatedProductStore = useSelector((state) => state.editService.editService);
        ...
        try {
            ...
            if (selectedImages) {
                ...
                if (uploadImages.data) {
                    ...
                    setTimeout(() => {
                        closeViewProduct();
                        navigate(`/newproduct/${response.data._id}/${response.data.name}`);
                    }, 3000);
                }
            }
