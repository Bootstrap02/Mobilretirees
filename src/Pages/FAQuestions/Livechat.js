...
/* ── Extract keywords from a user message and find matching FAQs ── */
...
const BOT_INTRO = {
  id: 'intro',
  from: 'bot',
  text: "Hello! 👋 I'm the EMRAN Help Assistant. Ask me anything about membership, benefits, the website, or any EMRAN matter and I'll find the most relevant answers for you.",
  faqs: [],
  time: new Date(),
};
    ...
    setTimeout(() => {
      ...
      const botMsg = {
        ...
        text: matches.length > 0
          ...
          : "I couldn't find a specific answer for that. Please try rephrasing, or visit our full FAQ page. You can also email emranannuitants@gmail.com for direct support.",
        faqs: matches,
        time: new Date(),
      };
      setTyping(false);
  ...
  return (
    <>
      <style>{`
        ...
        .chat-bubble-btn { transition: transform 0.2s, box-shadow 0.2s; }
        .chat-bubble-btn:hover { transform: scale(1.07); box-shadow: 0 8px 32px rgba(0,31,91,0.28); }
      ...
      <div className="chat-widget" style={{ position: 'fixed', bottom: 28, right: 28, zIndex: 1000 }}>

        {/* ── Chat Window ── */}
        {open && (
          <div className="chat-window-enter"
            style={{ position: 'absolute', bottom: 72, right: 0, width: 360, maxWidth: 'calc(100vw - 40px)', background: '#fff', borderRadius: 20, boxShadow: '0 12px 60px rgba(0,31,91,0.18)', display: 'flex', flexDirection: 'column', overflow: 'hidden', height: 520 }}>
                ...
                <div key={msg.id} className={msg.from === 'bot' ? 'chat-msg-in' : 'chat-msg-out'}
                  ...
                  {msg.from === 'bot' && (
                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, maxWidth: '90%' }}>
                      ...
                      <div>
                        <div style={{ background: '#f0f3fa', borderRadius: '4px 16px 16px 16px', padding: '10px 14px', fontSize: 13.5, color: '#1a2340', lineHeight: 1.55 }}>
                          {msg.text}
                        </div>
                            ...
                            {msg.faqs.map((faq, fi) => {
                              ...
                              return (
                                <div key={fi} style={{ background: '#fff', borderRadius: 12, border: '1px solid #e8ecf4', overflow: 'hidden' }}>
                                  ...
                                  {isOpen && (
                                    <div style={{ padding: '8px 12px 12px', fontSize: 12.5, color: '#4a5568', lineHeight: 1.6, borderTop: '1px solid #f0f3fa' }}>
                                      {faq.a}
                                    </div>
                                  )}
                                </div>
                  ...
                  {msg.from === 'user' && (
                    <div style={{ maxWidth: '78%' }}>
                      <div style={{ background: '#001F5B', borderRadius: '16px 4px 16px 16px', padding: '10px 14px', fontSize: 13.5, color: '#fff', lineHeight: 1.55 }}>
                        {msg.text}
                      </div>
                ...
                <div style={{ marginTop: 4 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5, paddingLeft: 36 }}>
                    {quickQuestions.map((q, i) => (
                      <button key={i} className="quick-q" onClick={() => { setInput(q); setTimeout(() => sendMessage(), 0); }}
            ...
            <div style={{ padding: '10px 12px', borderTop: '1px solid #f0f3fa', display: 'flex', gap: 8, flexShrink: 0 }}>
              <input ref={inputRef} className="chat-input" value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey}
                ...
                style={{ flex: 1, padding: '10px 14px', borderRadius: 24, border: '1.5px solid #e8ecf4', fontSize: 13.5, color: '#1a2340', background: '#f7f8fc', resize: 'none' }}
              />
        ...
        <button className="chat-bubble-btn" onClick={() => setOpen(o => !o)}
          style={{ width: 56, height: 56, borderRadius: '50%', background: open ? '#001F5B' : 'linear-gradient(135deg, #E30613 0%, #b30010 100%)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(227,6,19,0.35)', color: '#fff', fontSize: 24, transition: 'all 0.25s' }}>
          {open ? (
        ...
        {/* Unread dot */}
        {!open && (
          <div style={{ position: 'absolute', top: 2, right: 2, width: 10, height: 10, borderRadius: '50%', background: '#E30613', border: '2px solid #fff' }} />
        )}
      </div>
    </>
  );
